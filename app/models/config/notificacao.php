...
class ModelConfigNotificacao extends BaseModel{
    public function getAll(bool $ativos = false) {
        $q = sprintf('SELECT * FROM notificacao %s', $ativos === true ? 'WHERE ativo = 1' : '');
        return $this->db->query($q)->rows;
    }

    public function getById($idnotificacao) {
      $q = sprintf('SELECT * FROM notificacao WHERE idnotificacao = %s', (int)$idnotificacao);
      return $this->db->query($q)->row;
    }

    public function getByIdusuario($idusuario) {
      $q = sprintf('SELECT * FROM notificacao WHERE idusuario = %s', (int)$idusuario);
      return $this->db->query($q)->rows;
    }
    ...
    public function add($data) {
      ...
      $q = sprintf('INSERT INTO notificacao SET %s ', $change);
      $this->db->query($q);
      $data['idnotificacao'] = $this->db->getLastId();
      return $data;
    }

    public function edit($data) {
      if (!isset($data['idnotificacao'])) return false;
      $change = $this->db->formatChange($data);

      $q = sprintf('UPDATE notificacao SET %s WHERE idnotificacao = %s', $change, (int)$data['idnotificacao']);
      $this->db->query($q);
      return $this->db->countAffected() > 0 ? true : false;
    }
    
    public function delete($idnotificacao) {
      $q = sprintf('DELETE FROM notificacao WHERE idnotificacao = %s', (int)$idnotificacao);
      $this->db->query($q);
      return $this->db->countAffected() > 0 ? true : false;
    }
