...
class ModelProdutoProduto extends BaseModel{
    ...
    public function getByImagem($iddepartamento) {
      $q = sprintf('SELECT * FROM arquivo AS a INNER JOIN produto AS p ON (a.idproduto = p.idproduto) WHERE a.tipo = "arquivo_produto" AND a.nome = "imagem" AND p.iddepartamento = %s', (int)$iddepartamento);
      return $this->db->query($q)->row;
    }
